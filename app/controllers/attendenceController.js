...
const punchIn = async (req, res) => {
  ...
  try {
    ...
    const isAlreadyPunchedQuery =
      "SELECT * FROM attendence WHERE users_id = ? AND punch_in BETWEEN ? AND ?";
...
const punchOut = async (req, res) => {
  ...
  try {
    ...
    const punchOutQuery = "UPDATE attendence SET punch_out = CURRENT_TIME ";
    const [runPunchOutQuery] = await promisePool.query(punchOutQuery);

    return res
      .status(200)
      .json({ success: true, data: "punch-out successfully!",runPunchOutQuery });
  } catch (error) {
